...
| *Product version*
| In the footer at the bottom of the sign-in screen — visible without signing in — and on the *Admin* pages. An administrator can also read it from `GET /api/actuator/info`, which is authoritative even if a page was served from cache.
...
A few checks resolve the most common problems:
