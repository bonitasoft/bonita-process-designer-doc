...
. Enter a *username*. It is the unique login identifier and is stored in lower case. It must be 3–20 characters long and may contain only letters, digits, and underscores. The username cannot be changed after the account is created.
...
[IMPORTANT]
====
If you let {productName} generate the password, it is *shown only once*, immediately after creation. Copy it before you close the dialog and share it with the user through a secure channel. Whether you set the password or let it be generated, the new user must change it on first login.
====
