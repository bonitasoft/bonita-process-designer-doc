...
* An internet connection, required for AI generation features.
...
On the first login, use the default `admin` / `admin` account; you are prompted to change the password immediately. For the full initial setup flow (license, registration, AI configuration), see xref:installation:first-time-setup.adoc[First-time setup].
...
The `backend` service carries the application log — it is the one to read first when something misbehaves, and the one to export for a support request. To raise its verbosity, trace a single request, or cap how much disk the logs use, see xref:administration:logging.adoc[Logs and log levels].
