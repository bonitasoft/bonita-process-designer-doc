...
Visibility depends on the viewer's role within the group that owns the diagram.

[cols="1,1,1,1"]
|===
| Role | Unpublished diagram | Draft (current) | Published versions
...
The key rule: *a diagram with no published version is invisible to Readers.* A Reader only ever sees a diagram once at least one version has been published, and they always see the latest published version, never the in-progress draft.
